...
type MarketTracker struct {
	...
	trades map[types.OutputID]Trade // identified by parent ID of bid input
...
func findBids(height types.BlockHeight, txns []types.Transaction) (newBids []Bid, filledBids []types.Transaction) {
	for _, txn := range txns {
		if probablyFillTxn(txn) {
			filledBids = append(filledBids, txn)
		}
		for _, arb := range txn.ArbitraryData {
			if !bytes.HasPrefix(arb, BidPrefix) {
				continue
			}
			var bid Bid
			if err := encoding.Unmarshal(bytes.TrimPrefix(arb, BidPrefix), &bid); err != nil {
...
func (mt *MarketTracker) ProcessConsensusChange(cc modules.ConsensusChange) {
	...
	for _, b := range cc.AppliedBlocks {
		...
		for _, bid := range appliedBids {
			mt.trades[bid.ID] = Trade{Bid: bid}
		}
...
func (mt *MarketTracker) Bids() []Bid {
	mt.mu.Lock()
	defer mt.mu.Unlock()

	bids := make([]Bid, 0, len(mt.trades))
...
func (mt *MarketTracker) Trades() []Trade {
	mt.mu.Lock()
	defer mt.mu.Unlock()

	trades := make([]Trade, 0, len(mt.trades))
	for _, trade := range mt.trades {
		if trade.Height != 0 {
...
func (mt *MarketTracker) load() error {
	...
	mt.trades = make(map[types.OutputID]Trade)
...
func Start(dir string, siadAddr string) (*MarketTracker, error) {
	...
	siad.Address = siadAddr
	mt, err := newMarketTracker(dir)
	if err != nil {
		return nil, err
	}
	fmt.Printf("start value %s\n", mt.ccid)
	errCh, unsub := siad.ConsensusSetSubscribe(mt, mt.ccid, nil)
	mt.unsub = unsub
	mt.syncing = true
